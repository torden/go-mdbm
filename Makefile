...
all: clean setup build golanglint
...
ifeq ($(GOLANGV18_OVER),1)
	@$(CMD_GO) install github.com/Songmu/make2help/cmd/make2help@latest
endif
	...
	@$(CMD_GO) get mvdan.cc/gofumpt@latest
	@$(CMD_GO) install mvdan.cc/gofumpt@latest
	@$(CMD_GO) get github.com/golangci/golangci-lint/cmd/golangci-lint
	@$(CMD_GO) install github.com/golangci/golangci-lint/cmd/golangci-lint
...
coveralls::
	...
	@$(shell which goveralls) -coverprofile=$(PATH_REPORT)/raw/$(PATH_CONVER_PROFILE)
	@$(CMD_ECHO) -e "\033[1;40;36mDone\033[01;m\x1b[0m"
...
## Generate the report for profiling (required graphviz)
report: pprof
	@$(CMD_MKDIR) -p $(PATH_REPORT)/raw/ $(PATH_REPORT)/doc/
...
help::
ifeq ($(GOLANGV18_OVER),1)
	@$(shell which make2help) $(MAKEFILE_LIST)
else
	@$(CMD_ECHO) -e "\033[1;40;36mSKIP: your golang is older version $(shell go version)\033[01;m\x1b[0m"
endif
...
clean::
	...
	@$(CMD_RM) -rfv *.coverprofile *.swp *.core *.html *.prof *.test *.report ./$(PATH_REPORT)/* ./tmp/*.mdbm *.txt *.log *.out report
	@$(CMD_ECHO) -e "\033[1;40;36mDone\033[01;m\x1b[0m"

.PHONY: clean cover coveralls help golanglint lint pprof report run setup strictlint test build 
